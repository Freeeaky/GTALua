...
example_spawn_bodyguard = ScriptThread("example_spawn_bodyguard_2") 
...
function example_spawn_bodyguard:Run()
	while self:IsRunning() do
		...
		if IsKeyDown(0x78) then
			...
			blip = natives.UI.ADD_BLIP_FOR_ENTITY(ped_handle)
			print("blip : ", blip)
		...
		if IsKeyDown(0x79) then
			print("id: ", blip)
			print(natives.UI.DOES_BLIP_EXIST(blip))
			print("try removing blip")
			local h_mem = CMemoryBlock(4)
			h_mem:WriteInt32(0, blip)
			natives.UI.REMOVE_BLIP(h_mem)
			print("blip removed")
		end
		-- Wait
		self:Wait(50)
